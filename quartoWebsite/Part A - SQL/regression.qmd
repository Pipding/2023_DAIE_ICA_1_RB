...
Only completed projects are taken into account when performing linear regression.
...
correlation coefficient of `r cor.test(completedProjects$Budget, completedProjects$AssetCount)$estimate`. Predictably, team size has a moderate-to-strong positive correlation ( correlation coefficient `r sprintf("%4.3f", cor.test(completedProjects$Budget, completedProjects$AssetDevCount)$estimate)`) with project budget.
