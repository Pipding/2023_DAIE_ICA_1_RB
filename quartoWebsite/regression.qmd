...
A possible explanation for the inverse relationship between project budget and duration is the size of the dev team on a project. As explored in @fig-budgetvsteamsize, project budget is highly correlated with the number of developers working on the project. Developer count is also highly negatively correlated with project duration (correlation coefficient `r cor.test(completedProjects$AssetDevCount, completedProjects$Duration)$estimate`), as shown in @fig-projectdurationteamsize.
