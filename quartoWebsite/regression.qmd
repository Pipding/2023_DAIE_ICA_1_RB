...
# Generate a model which will be used to make predictions about the likelihood of missing milestones
model <- glm(MissedMilestone ~ Budget + AssetDevCount, family = binomial(link = "logit"), data = startedProjects)

projectOutcomeMatrix <- expand.grid(
  Budget = seq(
    min(startedProjects$Budget),
    max(startedProjects$Budget),
    length.out = 100),
  AssetDevCount = seq(
    min(startedProjects$AssetDevCount),
    max(startedProjects$AssetDevCount),
    length.out = 100)
  )

# Prediction: https://www.digitalocean.com/community/tutorials/predict-function-in-r
projectOutcomeMatrix$predictedMilestoneMisses <- predict(model, newdata = projectOutcomeMatrix, type = "response")

# Combine real data with predicted data
predictionsMerged <- cbind(startedProjects, predictedProbabilities = predict(model, type = "response"))

p <- ggplot(
  predictionsMerged,
  aes(x = Budget, y = AssetDevCount, color = predictedProbabilities)) +
  geom_point(aes(shape = MissedMilestone), size = 3) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(
    x = "Budget",
    y = "Dev Team Size",
    color = "Predicted Probability",
    shape = "Missed Milestone")
